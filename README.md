...
项目报告中的“调优随机森林 ROC-AUC 约 0.97、违约召回率约 0.88”作为报告亮点在 Streamlit 首页单独展示；当前仓库内的默认训练结果采用较保守的贷前特征集合，不使用贷后还款字段，因此 ROC-AUC 更接近真实贷前风控建模表现。
...
如果 `modeling/artifacts/random_forest_pipeline.joblib` 存在，平台会优先使用随机森林模型输出违约概率；如果模型文件不存在，则自动回退到内置评分卡规则。
...
当前 Streamlit 单客户与批量评估模块会优先读取 `modeling/artifacts/random_forest_pipeline.joblib` 进行模型推理；如果模型文件不存在，则自动回退到内置评分卡规则。离线训练模块仍保留逻辑回归、决策树、随机森林和梯度提升模型的对比结果，便于展示完整建模流程。
